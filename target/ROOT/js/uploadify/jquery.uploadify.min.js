...
(function(c) {
    var b = {init: function(d, e) {
            return this.each(function() {
                ...
                if (h) {
                    ...
                    i.queueData = {files: {},filesSelected: 0,filesQueued: 0,filesReplaced: 0,filesCancelled: 0,filesErrored: 0,uploadsSuccessful: 0,uploadsErrored: 0,averageSpeed: 0,queueLength: 0,queueSize: 0,uploadSize: 0,queueBytesUploaded: 0,uploadQueue: [],errorMsg: "文件上传异常！"};
                    i.original = m;
                    i.wrapper = l;
                    i.button = f;
                    i.queue = k;
    ...
    var a = {onDialogOpen: function() {
            var d = this.settings;
            this.queueData.errorMsg = "文件上传异常！";
            ...
            if (c.inArray("onSelectError", e.overrideEvents) < 0) {
                switch (g) {
                    ...
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\n文件 "' + d.name + '" 不能超过 (' + e.fileSizeLimit + ")限制.";
                        break;
                    case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
                        this.queueData.errorMsg += '\n文件 "' + d.name + '" 不能为空文件.';
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\n文件 "' + d.name + '" 不能为 (' + e.fileTypeDesc + ")类型.";
                        break;
                }
            }
